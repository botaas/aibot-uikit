...
export interface Client<OneChatGenerics extends ExtendableGenerics = DefaultGenerics> {
	 ...
	 */
	muteUser: (targetID: string, userID?: string, options?: MuteUserOptions<OneChatGenerics>) => Promise<MuteUserResponse<OneChatGenerics>>
	 ...
	 */
	unpinMessage: (messageOrMessageId: string | { id: string }, userId?: string | { id: string }) => Promise<UpdateMessageAPIResponse<OneChatGenerics>>
	...
	deleteMessage: (messageID: string, hardDelete?: boolean) => Promise<APIResponse & { message: MessageResponse<OneChatGenerics> }>
