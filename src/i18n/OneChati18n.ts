...
import type { TranslationLanguages, UnknownType } from '../types';
...
/**
 * Wrapper around [i18next](https://www.i18next.com/) class for OneChat related translations.
 * Instance of this class should be provided to Chat component to handle translations.
 * OneChat provides following list of in-built translations:
 * 1. English (en)
 * 2. Dutch (nl)
 * 3. Russian (ru)
 * 4. Turkish (tr)
 ...
 *
 * ```
 * const i18n = new OneChati18n({ language 'nl' });
 ...
 *
 * OneChat react chat components uses [dayjs](https://day.js.org/en/) internally by default to format datetime stamp.
 ...
 * language with OneChati18n (either via constructor or registerTranslation()) or you can provide your own Dayjs or Moment instance
 * to OneChati18n constructor, which will be then used internally (using the language locale) in components.
 *
 ...
 * })
 * ```
 * If you would like to stick with english language for datetimes in OneChat components, you can set `disableDateTimeTranslations` to true.
 *
 */
const defaultOneChati18nOptions = {
  DateTimeParser: Dayjs,
...
export class OneChati18n {
   ...
   *    Make sure to load all the required locales in this Moment or Dayjs instance that you will be provide to OneChati18n
   *
   * @param {*} options
   */
  constructor(options: Options = {}) {
    const finalOptions = {
      ...defaultOneChati18nOptions,
      ...options,
    };
    ...
    try {
      ...
      throw Error(
        `OneChati18n: Looks like you wanted to provide Dayjs instance, but something went wrong while adding plugins ${error}`,
      );
    }
    ...
    this.i18nextConfig = {
      ...
      parseMissingKeyHandler: (key) => {
        this.logger(`OneChati18n: Missing translation for key: ${key}`);
  ...
  registerTranslation(
    ...
    if (!translation) {
      this.logger(
        `OneChati18n: registerTranslation(language, translation, customDayjsLocale) called without translation`,
      );
      return;
    }
