...
> = {
  ...
  own_reactions?: ReactionResponse<OneChatGenerics>[];
...
const UnMemoizedSimpleReactionsList = <
  OneChatGenerics extends DefaultOneChatGenerics = DefaultOneChatGenerics
>(
  props: SimpleReactionsListProps<OneChatGenerics>,
) => {
  const { handleReaction: propHandleReaction, ...rest } = props;

  const { Emoji, emojiConfig } = useEmojiContext('SimpleReactionsList');
  const { handleReaction: contextHandleReaction } = useMessageContext<OneChatGenerics>(
    'SimpleReactionsList',
  );
