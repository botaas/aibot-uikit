...
>(
  ...
  const { channel } = useChannelStateContext<OneChatGenerics>('useFileUploads');
  const { addNotification } = useChannelActionContext<OneChatGenerics>('useFileUploads');
  const { getAppSettings } = useChatContext<OneChatGenerics>('useFileUploads');
