...
>(
  ...
  const { channel } = useChannelStateContext<OneChatGenerics>('useSubmitHandler');
  const { addNotification, editMessage, sendMessage } = useChannelActionContext<OneChatGenerics>(
    'useSubmitHandler',
  );
  ...
  const getAttachmentsFromUploads = () => {
    const imageAttachments = imageOrder
      ...
      .map<Attachment<OneChatGenerics>>(({ file: { name }, url, ...rest }) => ({
    ...
    const fileAttachments = fileOrder
      ...
      .filter((upload) => upload.state !== 'failed')
      .map<Attachment<OneChatGenerics>>((upload) => ({
    ...
    if (message) {
      ...
      try {
        await editMessage(({
          ...
          ...updatedMessage,
          ...customMessageData,
        } as unknown) as UpdatedMessage<OneChatGenerics>);
