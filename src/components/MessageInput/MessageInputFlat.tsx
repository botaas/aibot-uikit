...
>() => {
  const { quotedMessage } = useChannelStateContext<OneChatGenerics>('MessageInputFlat');
  const { setQuotedMessage } = useChannelActionContext('MessageInputFlat');
  const { channel, themeVersion } = useChatContext<OneChatGenerics>('MessageInputFlat');

  useEffect(() => {
    const handleQuotedMessageUpdate = (e: Event<OneChatGenerics>) => {
  ...
  return themeVersion === '2' ? (
    <MessageInputV2<OneChatGenerics> />
  ) : (
    <MessageInputV1<OneChatGenerics> />
  );
};
...
>() => {
  ...
  const {
    ...
    uploadNewFiles,
  } = useMessageInputContext<OneChatGenerics>('MessageInputFlat');
  ...
  } = useComponentContext<OneChatGenerics>('MessageInputFlat');
...
>() => {
  const {
    ...
    multipleUploads,
    quotedMessage,
  } = useChannelStateContext<OneChatGenerics>('MessageInputV2');
