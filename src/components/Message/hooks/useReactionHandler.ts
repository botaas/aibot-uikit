...
>(
  ...
  const { updateMessage } = useChannelActionContext<OneChatGenerics>('useReactionHandler');
  const { channel } = useChannelStateContext<OneChatGenerics>('useReactionHandler');
  const { client } = useChatContext<OneChatGenerics>('useReactionHandler');
  ...
  const createMessagePreview = useCallback(
    (
      ...
      } as OneChatMessage<OneChatGenerics>;
    },
  ...
  const toggleReaction = throttle(async (id: string, type: string, add: boolean) => {
    ...
    const newReaction = creatReactionPreview(type) as ReactionResponse<OneChatGenerics>;
    ...
    try {
      ...
      const messageResponse = add
        ? await channel.sendReaction(id, { type } as Reaction<OneChatGenerics>)
  ...
  return async (reactionType: string, event?: React.BaseSyntheticEvent) => {
    ...
    let userExistingReaction = (null as unknown) as ReactionResponse<OneChatGenerics>;
...
>(
  ...
  const {
    ...
    channelCapabilities = {},
    channelConfig,
  } = useChannelStateContext<OneChatGenerics>('useReactionClick');
