...
> = Omit<MessageProps<OneChatGenerics>, MessagePropsToOmit> &
  Pick<MessageContextValue<OneChatGenerics>, MessageContextPropsToPick> & {
    canPin: boolean;
...
const MessageWithContext = <
  OneChatGenerics extends DefaultOneChatGenerics = DefaultOneChatGenerics
>(
  props: MessageWithContextProps<OneChatGenerics>,
) => {
  const {
    canPin,
    groupedByUser,
  ...
  const { Message: contextMessage } = useComponentContext<OneChatGenerics>('Message');
  ...
  const messageContextValue: MessageContextValue<OneChatGenerics> = {
    ...rest,
    actionsEnabled,
...
>(
  ...
  const { highlightedMessageId, mutes } = useChannelStateContext<OneChatGenerics>('Message');
