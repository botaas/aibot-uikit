...
> = {
  ...
  /* The `OneChat` message object, which provides necessary data to the underlying UI components (overrides the value stored in `MessageContext`) */
  message?: OneChatMessage<OneChatGenerics>;
  /* Theme string to be added to CSS class names */
  theme?: string;
} & Pick<MessageContextValue<OneChatGenerics>, 'renderText'>;

const UnMemoizedMessageTextComponent = <
  OneChatGenerics extends DefaultOneChatGenerics = DefaultOneChatGenerics
>(
  props: MessageTextProps<OneChatGenerics>,
) => {
  const {
    customInnerClass,
    customWrapperClass = '',
  ...
  const { QuotedMessage = DefaultQuotedMessage } = useComponentContext<OneChatGenerics>(
    'MessageText',
  );
  ...
  const {
    ...
    unsafeHTML,
  } = useMessageContext<OneChatGenerics>('MessageText');
